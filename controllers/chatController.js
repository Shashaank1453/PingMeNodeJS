...
const postCreateChat = async (req, res) => {
    ...
    usernames.forEach(async (username) => {
        ...
        if (!data) {
            res.status(404).json({ msg: `${username} doesn't exists!` }).send()
        }
    })
    if (usernames.length == 1) {
        if (
            ...
            res.status(403).json({ msg: 'Chat Already Exists' })
        else {
            await ChatModel.create({ usernames: [creatorUsername, usernames[0]] })
    ...
    ChatModel.findById(chatId)
        .then(data => {
            MessageModel.create({ senderUsername, chatId, message })
                .then(messageData => { res.status(200).json(messageData) })
